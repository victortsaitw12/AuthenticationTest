...
| **Authentication（認證）** | 驗證用戶身份的過程，回答"你是誰"的問題 |
| **Authorization（授權）** | 檢查用戶權限的過程，回答"你可以做什麼"的問題 |
| **Authentication Scheme** | 定義認證方式的規則，如 JWT Bearer、Cookie、OpenID Connect 等 |
| **JWT Bearer Token** | 在 Authorization Header 中以 "Bearer <token>" 格式傳輸的 JWT Token |
| **Token 驗證參數** | ValidateIssuer、ValidateAudience、ValidateLifetime、ValidateIssuerSigningKey 等 |
| **[Authorize] 特性** | 標記端點需要認證，未認證用戶會收到 401 Unauthorized |
| **ClaimsPrincipal** | 代表認證用戶及其所有 Claims 信息的對象 |
| **認證中間件** | 在請求管道中驗證 Token 並設置 HttpContext.User 的中間件 |
| **授權中間件** | 在請求管道中檢查 [Authorize] 特性並驗證用戶權限的中間件 |
| **HttpContext.User** | 當前請求的認證用戶，包含 Identity 和 Claims 信息 |
| **IsAuthenticated** | 表示用戶是否已認證的布爾值屬性 |
...
## 第八步：端點安全保護（Authentication Scheme 和 [Authorize]）
